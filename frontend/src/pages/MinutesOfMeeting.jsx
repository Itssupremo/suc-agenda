...
function QuarterCard({ quarter, doc, sucId, sucName, year, onRefresh, onViewFile }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      fd.append('file', file);
      await uploadDoc(sucId, 'minutes', quarter, fd);
      setFile(null);
  ...
  const handleReset = async () => {
    ...
    setResetting(true);
    try {
      await resetDoc(sucId, 'minutes', quarter, year);
      setFile(null);
  ...
  return (
    <div className="agenda-quarter-card">
      <div className="agenda-quarter-header agenda-quarter-header--green">
        <span className="agenda-quarter-title" style={{ color: '#fff' }}>{quarter} Quarter</span>
...
function MinutesOfMeeting({ user }) {
  ...
  const fetchDocs = async () => {
    ...
    setLoading(true);
    try {
      const res = await getDocs(selectedSuc, year, 'minutes');
      setDocs(res.data);
    } catch {
      setDocs([]);
    } finally {
